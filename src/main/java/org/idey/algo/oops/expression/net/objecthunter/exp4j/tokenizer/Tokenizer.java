 ...
 */
package org.idey.algo.oops.expression.net.objecthunter.exp4j.tokenizer;

import java.util.Map;
import java.util.Set;

import org.idey.algo.oops.expression.net.objecthunter.exp4j.function.Function;
import org.idey.algo.oops.expression.net.objecthunter.exp4j.function.Functions;
import org.idey.algo.oops.expression.net.objecthunter.exp4j.operator.Operator;
import org.idey.algo.oops.expression.net.objecthunter.exp4j.operator.Operators;
    ...
    public Token nextToken(){
        ...
        if (Character.isDigit(ch) || ch == '.') {
            if (lastToken != null) {
                ...
                } else if (((lastToken.getType() != Token.TOKEN_OPERATOR || (lastToken.getType() != Token.TOKEN_OPERATOR
                            && ((OperatorToken)lastToken).getOperator().isLeftAssociative()
                            && ((OperatorToken)lastToken).getOperator().getNumOperands()==1))
            ...
            if (lastToken != null &&
                    ( (lastToken.getType() != Token.TOKEN_OPERATOR || (lastToken.getType() != Token.TOKEN_OPERATOR
                            && ((OperatorToken)lastToken).getOperator().isLeftAssociative()
                            && ((OperatorToken)lastToken).getOperator().getNumOperands()==1))
                            && lastToken.getType() != Token.TOKEN_PARENTHESES_OPEN
                            && lastToken.getType() != Token.TOKEN_FUNCTION
